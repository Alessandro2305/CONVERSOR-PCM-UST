...
async def escrever_no_pdf_original(
    ...
    try:
        ...
        if not col_ref or not col_item:
            raise HTTPException(status_code=400, detail="Colunas de Referência ou Código Item não encontradas no Excel.")
        ...
        # Cria a instância para ler o PDF base original
        ...
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf_plumber:
            for page_idx, original_page in enumerate(reader.pages):
                ...
                # Clona a página original intacta
                page_to_add = original_page

                # Se houver texto novo, mescla a camada transparente em cima da página original
                if escreveu_algo:
